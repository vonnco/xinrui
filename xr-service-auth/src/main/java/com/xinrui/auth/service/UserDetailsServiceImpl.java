...
import com.xinrui.auth.client.UserClient;
...
public class UserDetailsServiceImpl implements UserDetailsService {
    ...
    public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException {
        ...
        UserExt userExt = userClient.findUserByUsername(username);
        if(userExt == null){
            return null;
        }
        //取出正确密码（hash值）
        ...
        //从数据库获取权限
        List<Menu> permissions = userExt.getMenuList();
        if (permissions == null) {
            permissions = new ArrayList<>();
        }
        List<String> user_permission = new ArrayList<>();
        permissions.forEach(item-> user_permission.add(item.getMenuCode()));
